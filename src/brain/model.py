...
The DebateBrain: persona engine with:
- Persona prompts loaded from file
- Optional RAG quote retrieval
- Conversation memory (last 8 turns)
- Topic guidance across turns
- Azure/Qwen backend abstraction
"""
...
DEBATE_TOPICS = [
    "the economy, inflation, wages, and cost of living",
    "immigration, border enforcement, and asylum policy",
    "healthcare affordability, prescription drugs, and Medicare",
    "Ukraine, NATO, Middle East conflict, and deterrence",
    "Epstein file transparency and accountability for elite misconduct",
    "Hunter Biden pardon ethics and Department of Justice independence",
    "Trump legal exposure, election integrity, and rule of law",
    "China competition, trade resilience, manufacturing, and debt",
]
...
INTERJECTIONS = {
    ...
    "biden": [
        "C'mon, man.",
        "Look, that is just malarkey.",
        "Not true. Not a word of it is true.",
...
class DebateBrain:
    ...
    Supports:
    - "azure": Azure OpenAI
    - "qwen": local/HF Qwen 2.5 0.5B path
    """
    ...
    def __init__(self, persona: str, brain_type: str = "azure"):
        ...
        self.topic_index = 0
        self.turn_count = 0
        self.topic_rotation_turns = max(1, int(os.getenv("DEBATE_TOPIC_ROTATION_TURNS", "1")))
        self.rag_hits = 0
        self.rag_misses = 0
        ...
        persona_file = os.path.join(os.path.dirname(__file__), "personas", f"{persona}.txt")
        if os.path.exists(persona_file):
            with open(persona_file, "r", encoding="utf-8", errors="ignore") as f:
                self.persona_prompt = f.read()
        else:
            self.persona_prompt = (
                f"You are {persona.title()} in a presidential debate. "
                "Keep answers short, punchy, and in character."
            )
        ...
        try:
            ...
            logger.warning("[Brain:%s] RAG init error (%s) - running without retrieval", persona, exc)
            self.rag = None
    ...
    def _generate_azure_response(self, opponent_text: str) -> str:
        ...
        try:
            ...
            logger.error("[Brain:%s] Azure API error: %s", self.persona, exc)
    ...
    def get_opening_statement(self) -> str:
        return self.generate_response("Please give your opening statement for this presidential debate.")
    ...
    def _build_system_prompt(self, opponent_text: str) -> str:
        ...
        2. Current debate topic and response-shape guidance
        3. RAG block with matching quotes (if available)
        """
    ...
    def _get_rag_block(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning("[Brain:%s] RAG block error (non-fatal): %s", self.persona, exc)
            return None

    def _fallback_response(self) -> str:
        fallbacks = {
            "trump": "Look, I will tell you this - nobody knows more about this than me.",
            "biden": "Look, here is the deal - the American people deserve better than this.",
            "siskind": "Gentlemen, please. Let us keep this debate on track.",
        }
...
if __name__ == "__main__":
    ...
    reply_trump = trump.generate_response("The economy is struggling under your watch.")
    print(f"TRUMP: {reply_trump}\n")

    biden = DebateBrain("biden")
    reply_biden = biden.generate_response("Trump claims he had the greatest economy ever.")
    print(f"BIDEN: {reply_biden}\n")
